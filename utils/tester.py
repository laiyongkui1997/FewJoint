...
from utils.preprocessor import FewShotFeature, ModelInput, FewShotSupportFeature
from utils.device_helper import prepare_model
from utils.model_helper import make_model, load_model, make_finetune_model
from models.few_shot_slu import FewShotSLU
from models.finetune_slu import BertSLU
...
class FewShotTester(TesterBase):
    ...
    def eval_predictions(self, all_results: List[Dict[str, RawResult]], id2label_map: Dict[str, Dict[int, str]],
        ...
        if self.opt.task == 'slu':
            ...
            all_intent_pred_labels = [[id2label_map['intent'][pred_ids[0]] for pred_ids in batch_pred_ids]
                                      for batch_pred_ids in all_intent_pred_ids]
            all_intent_target_labels = [[self.to_list(feature.test_feature_item.data_item.label)[0]
                                         for feature in batch_features] for batch_features in all_intent_features]
            ...
            all_slot_pred_labels = [[[id2label_map['slot'][pred_id] for pred_id in pred_ids]
            ...
            for b_idx, (b_pred_labels, b_target_labels) in enumerate(zip(all_slot_pred_labels, all_slot_target_labels)):
                for i_idx, (pred_labels, target_labels) in enumerate(zip(b_pred_labels, b_target_labels)):
                    for p_label, t_label in zip(pred_labels, target_labels):
                        if p_label != t_label:
                            sentence_acc[b_idx][i_idx] = False
                            break

            sentence_acc = sentence_acc.astype(float)
            sentence_acc = np.mean(sentence_acc)
    ...
    def clone_model(self, model, id2label_map):
        ...
        slot_id2label, intent_id2label = id2label_map['slot'], id2label_map['intent']
        new_model: FewShotSLU
        old_model: FewShotSLU
...
class FineTuneTester(FewShotTester):
    """
        Support features:
            - multi-gpu [accelerating]
            - distributed gpu [accelerating]
    ...
    """
    def __init__(self, opt, device, n_gpu):
        super(FineTuneTester, self).__init__(opt, device, n_gpu)
    ...
    def get_data_loader(self, features):
        ...
        if self.opt.local_rank == -1:
            ...
            sampler = DistributedSampler(dataset)
        pad_collate = PadCollate(dim=-1, sp_dim=-2, sp_item_idx=[3])  # nwp_index, spt_tgt need special padding
        data_loader = DataLoader(dataset, sampler=sampler, batch_size=self.batch_size, collate_fn=pad_collate)
        return data_loader
    ...
    def unpack_feature(self, feature: FewShotFeature) -> List[torch.Tensor]:
        ret = [
            torch.LongTensor([feature.gid]),  # 0
            # support
            feature.support_input.token_ids,  # 1
            feature.support_input.segment_ids,  # 2
            feature.support_input.nwp_index,  # 3
            feature.support_input.input_mask,  # 4
            feature.support_input.slot_output_mask,  # 5
            feature.support_input.intent_output_mask,  # 6
            # target
            feature.slot_support_target,  # 7
            feature.intent_support_target,  # 8
            # Special
            torch.LongTensor([len(feature.support_feature_items)]),  # 9, support num
        ]
        return ret
    ...
    def clone_model(self, model, id2label_map):
        ...
        label2id_map = {key: {v: k for k, v in values.items()} for key, values in id2label_map.items()}

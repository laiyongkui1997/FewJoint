...
from models.modules.seq_labeler import SequenceLabeler, RuleSequenceLabeler, CRFSequenceLabeler
...
from models.few_shot_slu import FewShotSLU, SchemaFewShotSLU, SplitMetricFewShotSLU, EmissionMergeIntentFewShotSLU, \
    EmissionMergeSlotFewShotSLU, EmissionMergeIterationFewShotSLU
...
from models.finetune_slu import SlotClassifyLayer, IntentClassifyLayer, BertSLU
...
def make_finetune_model(opt, config):
    """ build fine-tuning model with same embedder but different decoder """
    ...
    slot_decoder = None
    if opt.task in ['slot_filling', 'slu']:
        slot_decoder = SlotClassifyLayer(emb_dim, len(label2id['slot']), label2id['slot'])

    if opt.task in ['intent', 'slu']:
        intent_decoder = IntentClassifyLayer(emb_dim, len(label2id['intent']), label2id['intent'])
